...
static bool isSSE4() {
	...
	return sse4_1Supportted && sse4_2Supportted;
}
...
void*  (*AVX_memcpy)(void* dest, void const* src, size_t numbytes) = AVX_memcpy_chk;
void* __cdecl AVX_memcpy_chk(void* dest, void const* src, size_t numbytes) {
#ifdef     __AVX__        
    if (isAVX())
        AVX_memcpy = AVX_memcpy_AVX;
    else
        AVX_memcpy = memcpy;
#else
	if(isSSE4())
		...
		AVX_memcpy = memcpy;
#endif
    return AVX_memcpy(dest, src, numbytes);
}


// Default (8-bit, 1 byte at a time)
void * memcpy8 (void *dest, const void *src, size_t len)
{
...
//
// This file provides a highly optimized version of memcpy8.
...
{
  void * returnval = dest; // memcpy8 is supposed to return the destination
  ...
  // NOTE: Each memcpy8 has its own loop so that any one can be used individually.
  {
    if(numbytes < 2) // 1 byte
    {
      memcpy8(dest, src, numbytes);
      offset = numbytes & -1;
...
void * AVX_memcpy_AVX(void *dest, void const*src, size_t numbytes)
{
  void * returnval = dest;
    ...
    ) // Check alignment
  {
    ...
    {
      memcpy_large_as(dest, (void*)src, numbytes);
    }
    else
    {
      memcpy_large_a(dest, (void*)src, numbytes); // Even if numbytes is small this'll work
    }
  }
  else // Unaligned
  {
    ...
    if(numbytes > numbytes_to_align)
    {
      ...
      memcpy_large(dest, (void*)src, numbytes_to_align);
    ...
    {
      memcpy_large(dest, (void*)src, numbytes);
    }
  }
